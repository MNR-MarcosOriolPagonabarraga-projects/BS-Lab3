...
\begin{figure}[!h]
    ...
    \caption*{Figure 2: Workflow done to achieve the exercise.}
\end{figure}
...
The \textbf{obtained results} match very well with what is \textbf{described in the literature}.
Showing a \textbf{clear correlation with Power Sprectral Density and activity} in each specific 
case. 
...
Both \textbf{Welch Periodogram and AR (MDL) PSD methods perform very similarly}. However, by observing 
topogram representations of PSD power/electrode, we think that the \textbf{AR method} is more \textbf{prone to 
saturate} (giving less visual evidence of differences across electrodes), whereas the \textbf{Welch 
method seems more balanced}.

No specific band based on the topograms from 3.1. results shows difference in deep sleep stages. All the results in
the AR (MDL) and Welch methods appear to have very similar outputs for each band.
